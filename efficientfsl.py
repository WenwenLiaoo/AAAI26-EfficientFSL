...
def forward_features(self, x):
    ...
    for i in range(layer_num):
        if i==5:
            ...
            prompt = self.efficientfsl.q[i].unsqueeze(0).expand(bs, -1, -1) # bs, num, feat_size
            if i == 0:
                prompt = self.efficientfsl.prompt_ffn[i](prompt)
            else:
                prompt = self.efficientfsl.prompt_ffn[i](prompt+q_feat[-1]*self.efficientfsl.feat_scale)
            prompt = self.efficientfsl.active_block[i](prompt)
            prompt = self.efficientfsl.prompt_drop(prompt)
            prompt_len = prompt.shape[1]
            prompt, x, h_prompt, hh_prompt = self.blocks[i](prompt, x)
        else:
            bs = x.shape[0]
            prompt, x, h_prompt, hh_prompt = self.blocks[i](x, x)
            prompt_len = prompt.shape[1]
        if prompt_len > 1:
            prompt = prompt.mean(dim=1).view(bs, 1, feat_size)
            h_prompt = h_prompt.mean(dim=1).view(bs, 1, feat_size)
            hh_prompt = hh_prompt.mean(dim=1).view(bs, 1, feat_size)
        q_feat.append(prompt)
        h_feat.append(h_prompt)
        hh_feat.append(hh_prompt)

    q_feat = torch.cat([q_feat[-1]] + q_feat + h_feat + hh_feat, dim=1)
    q_feat = self.efficientfsl(q_feat)  # bs feat_size
    q_feat = self.efficientfsl.head_norm(q_feat)
...
class Mlp1(nn.Module):
    def __init__(self, in_features=feat_size, out_features=feat_size, act_layer=nn.GELU, drop=0.1):
        super().__init__()
...
class Adapter(nn.Module):
    def __init__(self, hidden=64, drop=0.1):
        super().__init__()
        self.mlp = Mlp(feat_size, hidden, feat_size, nn.GELU, drop)
...
class ACTIVEBlock(nn.Module):
    def __init__(self, attn_scale=0.1, ffn_scale=0.1, drop=0.1):
        super().__init__()
        self.attn = ACTIVEAttention(attn_scale,drop=drop)
...
class ACTIVEAttention(nn.Module):
    def __init__(self, attn_scale, drop=0.1):
        ...
        self.q_norm = nn.LayerNorm(feat_size)
        self.kv_norm = nn.LayerNorm(feat_size)
...
class ConditionCombineBlock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.get_weight = Mlp1(feat_size, self.dim-1)
        self.act = nn.Tanh()
...
class SQAttention(nn.Module):
    def __init__(self, test_way, shot):
        super().__init__()
        self.test_way = test_way
        self.shot = shot
        self.sq_co = nn.Parameter(torch.randn(1))
        self.project_q = Adapter(48, 0.1)

    def forward(self, x):
        n_support = self.test_way * self.shot
        support = x[:n_support] 
        query = x[n_support:]
        x_s = support
        query_p = self.project_q(query)

        coefficent_sq = torch.matmul(support, query_p.T)
        coefficent_sq = F.softmax(coefficent_sq, dim=1)
        sq = coefficent_sq @ query   

        support = sq * self.sq_co + support * (1-self.sq_co)
        
        return x_s,support,query

class E_FSL(nn.Module):
    def __init__(self, args, num=16):
        super().__init__()

        self.q = nn.Parameter(torch.ones(layer_num, args.token_nums, feat_size))
        nn.init.uniform_(self.q, -1, 1)
        self.prompt_drop = nn.Dropout(0.1)
        
        self.active_block = nn.Sequential(
            *[ACTIVEBlock(args.attn_scale, args.ffn_scale) for _ in range(layer_num)])

        self.prompt_ffn = nn.Sequential(
            *[Adapter(48, 0.1) for _ in range(layer_num)])
        
        self.feat_scale = nn.Parameter(torch.ones(1) * args.feat_scale)

        self.out_dim = 3*layer_num+1
        self.mlp = Adapter(48, 0.1)
        self.out_combine = ConditionCombineBlock(dim=self.out_dim)
        self.out_norm = nn.LayerNorm(feat_size)

        self.attn_scale = args.attn_scale

        self.head_norm = nn.LayerNorm(feat_size)
        self.drop_path = DropPath(drop_prob=0.3)
        
        self.sqatt = SQAttention(args.test_way, args.shot)
...
def forward_vit(self, x):
    ...
    x,support,query = self.efficientfsl.sqatt(x)

    return x,support,query

def set_efficientfsl(args, model):
    global feat_size, layer_num
    if type(model) == timm.models.vision_transformer.VisionTransformer:
        ...
        model.efficientfsl = E_FSL(args)
        with torch.no_grad():
            model.efficientfsl.head_norm.weight.set_(model.norm.weight.detach())
            model.efficientfsl.head_norm.bias.set_(model.norm.bias.detach())
...
def forward_ref_features(self, x):
    ...
    for i in range(len(self.blocks)):
        ...
        if self.token_mode == "mean":
            ref_features.append(x.mean(dim=1))  # bs, feat_size
        elif self.token_mode == "cls":
            ref_features.append(x[:, 0])
        else:
            raise RuntimeError("Unknown token type")
    ref_features = torch.stack(ref_features, dim=1)  # bs, layer_num, feat_size
    return ref_features
